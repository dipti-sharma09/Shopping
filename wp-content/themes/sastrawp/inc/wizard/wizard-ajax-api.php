...
function tmpcoder_get_recommended_plugins(){
        ...
        if ( !empty($tgmpaClass->plugins) ) {
            foreach( $tgmpaClass->plugins as $plugKey => $plugin ){
                ...
                if ( tmpcoder_is_plugin_installed( $plugin_file_path ) && in_array($plugin_file_path, apply_filters('active_plugins', get_option('active_plugins'))) ){
                    ...
                    $next_step = __('Install & Activate', 'sastrawp');
                }else{
                    $next_step = __('Install & Activate', 'sastrawp');
                }
    ...
    if ( !empty($plugins) ){
        ...
        wp_send_json_success(
            array(
                'plugins'=> $plugins,
                'message'=> __('Plugins getting successfully.','sastrawp'),
                'next_step'=> $next_step,
            )
        );
    }else{
        $error = __('No recommended plugins found','sastrawp');
...
function tmpcoder_install_recommended_plugins(){
    ...
    if ( empty($error) ){
        ...
        echo wp_json_encode( array('success'=> true, 'data' => array("message"=> __('All recommended plugins installed & activated successfully.','sastrawp') ) ) );
        exit;

    }else{
        $error = implode(', ', $error). __(' Could not install','sastrawp');
        wp_send_json_error(array('message'=> $error ));
    }
}
...
function tmpcoder_get_pro_plugin_info(){
    ...
    ob_start();
        echo wp_kses_post(sprintf(
            ...
            '<h2 class="wizard-heading">%s</h2>', __("Get Sastra Addons Pro", 'sastrawp')));
            ...
            <span><?php echo esc_html__( 'Get Pro Now', 'sastrawp' ); ?></span>

            <?php
            echo "</a>";

            echo '<div class="next-step-action">';
            echo '<a href='.esc_url($admin_url).' class="button button-primary next-step-btn">'.esc_html__('Done', 'sastrawp').'</a>';
            echo '</div>';
