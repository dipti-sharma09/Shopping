...
function tmpcoder_theme_admin_enqueue_script_custom(){
    ...
    if ( isset($current_screen->base) && ($current_screen->base == 'admin_page_tmpcoder-theme-wizard' || $current_screen->base == 'appearance_page_tmpcoder-theme-wizard') ){
        ...
        $my_data = array(
            ...
            'next_step_btn' => esc_html__("Next Step",'sastrawp'),
            'required_plugin_installing' =>  esc_html__("Installing Recommended Plugins",'sastrawp'),
            'getting_required_plugins' =>  esc_html__("Getting Recommended Plugins Info",'sastrawp'),
            'install_required_plugins' =>  esc_html__("Install Recommended Plugins",'sastrawp'),
            ...
            'install_and_activate' =>  esc_html__("Install & Activate",'sastrawp'),
            'installed_and_activate' =>  esc_html__("Installed & Activate",'sastrawp'),
            'installed_and_activated' =>  esc_html__("Activated",'sastrawp'),
            'loading_pro_plugin_info' =>  esc_html__("Pro Plugin Info Loading...",'sastrawp'),
            'network_error' => esc_html__("check network connection, try again.",'sastrawp'),
        );
...
class Tmpcoder_Theme_Setup_Wizard {
    ...
    function wizard_admin_notice_success() {
        if ( isset($_GET['saved']) && $_GET['saved'] == "wizard" ){ // phpcs:ignore WordPress.Security.NonceVerification.Recommended    
            ...
            <div class="notice notice-success is-dismissible">
                <p><?php esc_html_e( 'Congrats, The Setup Wizard has successfully set up your website.', 'sastrawp' ); ?></p>
            </div>
            <?php
        }
    }
    ...
    function register_newpage(){
        add_theme_page(
            ...
            [$this, 'tmpcoder_theme_func'] // Function to display the page
        );        
    }
                ...
                <ul class="nav-tab-wrapper theme-wizard-nav wp-clearfix">
                    <li class="nav-tab theme-welcome" data-tab="theme-welcome">
                        <span class="step-number">1</span><?php echo esc_html( sprintf(
                          ...
                          __( 'Welcome %s Theme', 'sastrawp' ) , ucfirst( TMPCODER_THEME_NAME )) ); ?>
                    </li>
                    <li class="nav-tab install-plugins disabled" data-tab="install-plugins">
                        <span class="step-number">2</span><?php esc_html_e('Install Recommended Plugins', 'sastrawp'); ?>
                    </li>
                    <li class="nav-tab get-pro-plugins disabled" data-tab="get-pro-plugins">
                        <span class="step-number">3</span><?php esc_html_e('Get a Sastra Addons Pro', 'sastrawp'); ?>
                    </li>                  
                </ul>
                <div id="theme-welcome" class="tab-content tab-content-theme-welcome active">
                    <div class="tmpcoder-message-box theme-install-part">
                        ...
                        <h2 class="wizard-heading"><?php echo esc_html( sprintf(
                          ...
                          __( 'Welcome To %1$s Theme Wizard, %2$s!', 'sastrawp' ) , ucfirst( TMPCODER_THEME_NAME ), ucfirst( $display_name ) ) ); ?></h2>
                        <p class="wizard-title-text"><?php echo esc_html( sprintf( 
                            /* translators: %s is Theme Name. */
                            apply_filters( 'theme_admin_setup_welcome_text', __( 'We recommend making use of the %s Theme Wizard to create your website. the easiest way to get started.', 'sastrawp' ) ) , TMPCODER_THEME_NAME) ); ?>
                        </p>
                        <div class="next-step-action">
                            <button class="button button-primary next-step-btn"><?php esc_html_e('Next','sastrawp'); ?></button>
                            <a class="skip-theme-wizard"><?php esc_html_e('Skip Setup & Go to Dashboard','sastrawp' ); ?></a>
                        </div>
                    </div>
                </div>
