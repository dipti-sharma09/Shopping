...
$args = array(
    ...
    'page_parent' => 'sastra-welcome',
    //Specify if the admin menu should appear or not. Options: menu or submenu (Under appearance only)
    'allow_sub_menu'       => true,
    // Show the sections below the admin menu item or not
    'menu_title'           => esc_html__( 'Global Options','sastrawp' ),
    'page_title'           => esc_html(sprintf(/* translators: %s is the theme name */esc_html__('%s Settings', 'sastrawp'),TMPCODER_THEME_NAME)),
    ...
    'page_slug'            => 'sastra_addon_global_settings',
    ...
    $output = '<header class="tmpcoder-global-options-header">
                <div>
                    <div class="tmpcoder-global-options-logo">
                        ...
                        <h1>'.esc_html__( 'Global Options', 'sastrawp' ).'</h1>
                    </div>
                </div>';
    if (!defined( 'TMPCODER_ADDONS_PRO_VERSION' )) {
        ...
        $output .= '<div class="tmpcoder-upgrade-now-button-1">
                   <a target="_blank" href="'.esc_url( TMPCODER_PURCHASE_PRO_URL.'?ref=tmpcoder-theme-global-options' ).'" class="btn-link">
                        ...
                        <span>'.esc_html__( 'Get Pro Now', 'sastrawp' ).'</span>
                    </a>
                </div>';
    }
