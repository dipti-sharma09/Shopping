...
class Tmpcoder_Welcome_Notice {
	...
	*/
	public function __construct() {
		...
		$this->errors = array(
			'permission' => __( 'Sorry, you are not allowed to do this operation.','sastrawp' ),
			'nonce'      => __( 'Nonce validation failed','sastrawp' ),
			'default'    => __( 'Sorry, something went wrong.','sastrawp' ),
			'invalid'    => __( 'No post data found!','sastrawp' ),
		);

		// Render Notice
		add_action( 'admin_notices', [$this, 'tmpcoder_render_notice'] );

		// Enque AJAX Script
		add_action( 'admin_enqueue_scripts', [$this, 'tmpcoder_admin_enqueue_scripts'], 5 );

		add_action( 'wp_ajax_tmpcoder_dismissed_handler', [$this, 'tmpcoder_dismissed_handler'] );

		// Reset
		add_action( 'switch_theme', [$this, 'tmpcoder_reset_notices'] );
		add_action( 'after_switch_theme', [$this, 'tmpcoder_reset_notices'] );

		// Install Plugins

		add_action( 'wp_ajax_tmpcoder_cancel_elementor_redirect', [$this, 'tmpcoder_cancel_elementor_redirect'] );

		add_action( 'wp_ajax_tmpcoder_recommended_plugin_activate', [$this, 'tmpcoder_required_plugin_activate'] );
	}
...
// 		if ( 'sastra-welcome' !== $screen->parent_base ) 
// 		{
...
// 		}
				...
				<?php
					echo esc_html(
					    sprintf(
					        ...
					        esc_html__('Welcome to %s', 'sastrawp'),
					        TMPCODER_THEME_NAME
					    )
					);
				?>
			...
			<p>
				<?php
					echo sprintf(
					    ...
					    esc_html__( '%1$s comes with %2$s with various designs to pick from.', 'sastrawp' ),
					...
					echo sprintf(
					    ...
					    esc_html__('%s and many other Elementor Extensions & Effects.', 'sastrawp'),
					    '<strong><a href="' . esc_url(TMPCODER_SASTRA_ADDONS_WIDGETS_URL) . '" target="_blank" rel="noopener noreferrer">100+ Premium Elementor Widgets</a> like Post and Product Grid, Slider, Menu, Theme & Woocommerce Shop Builder</strong>'
					);
					
					esc_html_e('Easily Customize every fundamental part of your WordPress site with SastraWP Theme & Woocommerce Shop Builder.', 'sastrawp');
				
					echo sprintf(
					    /* translators: %s is the list of customizable elements */
					    esc_html__('Including your %s etc.', 'sastrawp'),
					...
					echo sprintf(
					    ...
					    esc_html__('Build your site with SastraWP and our site builder plugins %s.', 'sastrawp'),
					    '<strong>Elementor, Sastra Essential Addons For Elementor & Redux Framework</strong>'
					);
				?>
			</p>
			<div class="action-buttons">
				<a href="<?php echo esc_attr($redirect_url); ?>" class="button button-primary tmpcoder-install-recommended-plugin" data-action="<?php echo esc_attr($action); ?>">
					...
					echo sprintf(
					    ...
					    esc_html__('Get Started with Prebuild Demos %s', 'sastrawp'),
	...
	*/
	...
	public function tmpcoder_admin_enqueue_scripts( $page ) {
		...
		// Enqueue Scripts
		wp_enqueue_script( 'tmpcoder-welcome-notice-js', get_template_directory_uri() . '/inc/activation/js/welcome-notice'.tmpcoder_min_suffix().'.js', ['jquery', 'updates'], false, true );

		wp_localize_script( 'tmpcoder-welcome-notice-js', 'tmpcoder_localize', [
				'failed_message' => esc_html__( 'Something went wrong, contact support.','sastrawp' ),
				'ajaxUrl' => admin_url( 'admin-ajax.php' ),
				'tmpcoderSitesLink' => admin_url( 'themes.php?page=starter-templates' ),
				'tmpcoderRedirectionLink' => admin_url( 'admin.php?page=sastra-welcome' ),
				'recommendedPluiginActivatingText'   => __( 'Activating','sastrawp' ) . '&hellip;',
				'recommendedPluiginDeactivatingText' => __( 'Deactivating','sastrawp' ) . '&hellip;',
				'recommendedPluiginActivateText'     => __( 'Activate','sastrawp' ),
				'recommendedPluiginDeactivateText'   => __( 'Deactivate','sastrawp' ),
				'recommendedPluiginSettingsText'     => __( 'Settings','sastrawp' ),
				'tmpcoderPluginManagerNonce'         => wp_create_nonce( 'sastra_plugin_manager_nonce' ),
		...
		// Enqueue Styles.
		wp_enqueue_style( 'tmpcoder-welcome-notice-css', get_template_directory_uri() . '/inc/activation/css/welcome-notice'.tmpcoder_min_suffix().'.css' );
	}
	...
	*/
	...
	public function tmpcoder_required_plugin_activate() {
		...
		$plugin_init = ['elementor/elementor.php','sastra-essential-addons-for-elementor/sastra-essential-addons-for-elementor.php','redux-framework/redux-framework.php'];
		...
		wp_send_json_success(
			array(
				'success' => true,
				'message' => __( 'Plugin Successfully Activated','sastrawp' ),
			)
		);
	}
