...
class Tmpcoder_Main_Class 
{
    ...
    */
    ...
    {   
        ...
        */
        ...
        add_action( 'after_setup_theme', [$this,'tmpcoder_setup'] );
        add_action( 'after_setup_theme', [$this,'tmpcoder_content_width'], 0 );

        /*
         * Enqueue css and js files
        */

        add_action( 'wp_enqueue_scripts', [$this,'tmpcoder_enqueue_styles_scripts'] );

        /* For Menu Section */

        add_filter( 'walker_nav_menu_start_el', [$this,'tmpcoder_add_plus_icon_menu'],10,4);
        add_filter('wp_nav_menu',[$this,'tmpcoder_add_class_in_submenu']); 
        ...
        add_action( 'tgmpa_register', [$this,'tmpcoder_require_plugins'] );
        }
        
        add_filter( 'body_class', [$this, 'tmpcoder_import_demo_body_class'] );

        add_action( 'tmpcoder_wp_body_open', [$this,'tmpcoder_load_preloder'] );
    }
    ...
    function tmpcoder_setup()
    {
        /*
            ...
            * If you're building a theme based on SastraWP, use a find and replace
            * to change 'sastrawp' to the name of your theme in all the template files.
        */

        load_theme_textdomain( 'sastrawp', get_template_directory() . '/languages' );
        ...
        register_block_pattern(
            'sastrawp/hello-world',
            array(
                'title'   => __( 'Section','sastrawp' ),
        ...
        register_nav_menus(
            array(
                'primary' => esc_html__( 'Primary','sastrawp' ),
                'footer_menu' => __( 'Footer Menu','sastrawp' ),
            )
        );
    ...
    */
    ...
    {
        if ( is_rtl() ){
            wp_enqueue_style( 'tmpcoder-style-rtl', get_template_directory_uri().'/style-rtl'.tmpcoder_min_suffix().'.css', array(), TMPCODER_THEME_CORE_VERSION );
        }
        
        wp_enqueue_style('tmpcoder-google-fonts', 'https://fonts.googleapis.com/css?family=Poppins:400,500,600,700&display=swap');

        wp_enqueue_style('tmpcoder-theme', get_template_directory_uri().'/theme'.tmpcoder_min_suffix().'.css', array(), TMPCODER_THEME_CORE_VERSION, false);

        wp_enqueue_style( 'tmpcoder-style', get_stylesheet_uri(), array(), TMPCODER_THEME_CORE_VERSION );

        wp_enqueue_style('tmpcoder-wc-style', get_template_directory_uri() . '/assets/css/wc-style'.tmpcoder_min_suffix().'.css', array(), TMPCODER_THEME_CORE_VERSION, false  );        
        wp_enqueue_script('tmpcoder-custom-script', get_template_directory_uri().'/assets/js/script'.tmpcoder_min_suffix().'.js', array('jquery'), TMPCODER_THEME_CORE_VERSION,true);

        wp_localize_script( 'tmpcoder-custom-script', 'tmpcoderAjaxObject', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
    ...
    function tmpcoder_require_plugins() {
        ...
        $plugins = array(               
            array(
                'name'     => esc_html__( 'Elementor','sastrawp' ),
                'slug'     => 'elementor',
            ),
            array(
                'name'     => esc_html__('Sastra Essential Addons for Elementor','sastrawp'), 
                'slug'     => 'sastra-essential-addons-for-elementor', 
            ),
            array(
                'name'     => esc_html__( 'Redux Framework','sastrawp' ),
...
if ( ! class_exists( 'Tmpcoder_Site_Settings' ) ):
    class Tmpcoder_Site_Settings {
        ...
        static public function tmpcoder_get( $key ) {
            global $tmpcoder_global_theme_options_sastrawp;
            if ( ! isset( $tmpcoder_global_theme_options_sastrawp ) ) {
                return null;
            }
            return isset( $tmpcoder_global_theme_options_sastrawp[ $key ] ) ? $tmpcoder_global_theme_options_sastrawp[ $key ] : null;
        }
        static public function tmpcoder_get_all_data() {
            ...
            return $tmpcoder_global_theme_options_sastrawp;
        }
    }
endif;
    ...
    ?>
        <div class="blog-list-box tmpcoder-blog-list">
            ...
            <div class="blog-list-content">
                ...
                <a href="<?php echo esc_url( get_permalink() ) ?>" class="read-more"><?php esc_html_e('Read More','sastrawp') ?></a>
            </div>
        </div>
...
function tmpcoder_posts_pagination() {
    ...
    $links = paginate_links( array(
        ...
        'prev_text' => __( '<i class="fa fa-angle-left" aria-hidden="true"></i>','sastrawp' ),
        'next_text' => __( '<i class="fa fa-angle-right" aria-hidden="true"></i>','sastrawp' ),
        'type'      => 'list',
    ));
...
function tmpcoder_register_block_styles() {
    ...
    if ( function_exists( 'register_block_style' ) ) {
        ...
        register_block_style(
            ...
            array(
                'name'  => 'bottom-right',
                'label' => __( 'Bottom Right','sastrawp' ),
            )
        );
...
if ( !function_exists('tmpcoder_get_theme_logo') ){
    ...
    function tmpcoder_get_theme_logo(){
        if ( class_exists('Tmpcoder_Site_Settings') ){
            $site_logo = ( Tmpcoder_Site_Settings::tmpcoder_has('tmpcoder_logo_image') ) ? Tmpcoder_Site_Settings::tmpcoder_get('tmpcoder_logo_image')['url'] : '';
            return $site_logo;
        }else{
            return '';
        }
