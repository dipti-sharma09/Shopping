...
/**
 * SastraWP functions and definitions
 *
...
define('TMPCODER_THEME_NAME', $theme->get( 'Name' ));

if ( ! defined( 'TMPCODER_THEME_SLUG' ) ) {
    define( 'TMPCODER_THEME_SLUG', $theme->get( 'TextDomain' ) );
}

if ( ! defined( 'TMPCODER_THEME_CORE_VERSION' ) ) {
    define( 'TMPCODER_THEME_CORE_VERSION', trim($theme->get('Version')) );
}
...
if ( !defined('TMPCODER_SASTRA_ADDONS_WIDGETS_URL') ){
    define('TMPCODER_SASTRA_ADDONS_WIDGETS_URL', 'https://sastraessentialaddons.com/widgets');
}
...
function tmpcoder_display_php_version_notices(){
    ...
    if ( null === $php_version ) {
        ...
        if ( version_compare( $php_version, '7.4' ) >= 0 ) {
            ...
            printf(
                ...
                esc_html__('Your site is running on an outdated version of PHP %s. The minimum recommended version of PHP is 7.4.', 'sastrawp'),
                esc_html($php_version)
            );
            echo '<a href="'.esc_url(admin_url().'?page=sastra-welcome&tab=system-info').'">'. esc_html('See more details') .'</a>';
            echo '</p></div>';
        }
    }
    ...
    if ( $memory < 128000000 ) : 
        ...
        printf(
            ...
            esc_html__( '%1$s - We recommend setting memory to at least <strong>128MB</strong>. Please define memory limit in <strong>wp-config.php</strong> file. To learn how, see: <a href="%2$s" target="_blank" rel="noopener noreferrer">Increasing memory allocated to PHP.</a>', 'sastrawp' ),
